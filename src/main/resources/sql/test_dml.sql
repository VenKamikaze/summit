...
insert into template select nextval('spare_seq'), 'HTML for a table in a tabular report with Mustache Templating', 'org.awiki.kamikaze.summit.service.processor.result.SourceProcessorResultTable', '<script type="text/javascript">function doReport() { var template="<table id=\"mustache_table\">{{#header}}<tr class=\"tablerowheader\">{{#cells}}<td class=\"tablecell\">{{value}}</td>{{/cells}}</tr>{{/header}}</table>"; var data = $.ajax({url: "/summit/api/filter/1/2.json?search=1", cache:false , success: function(response){ var html = Mustache.to_html(template, response); $("#mustacheReportRegion").html(html); }} ); }; $( document ).ready(doReport()); </script>';
