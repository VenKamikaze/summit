...
insert into template select nextval('spare_seq'), 'HTML for a table in a tabular report with Mustache Templating', 'org.awiki.kamikaze.summit.service.processor.result.SourceProcessorResultTable', '<script type="text/javascript"> function select(id) { parent.window.location.href = "##__CONTEXTPATH__##/view/" + id; }; </script><link type="text/css" rel="stylesheet" media="all" href="##__CONTEXTPATH__##/styles/table.css" /><div style="clear: both;"></div><div class="searchBar"><div class="search"><form id="searchForm" action="##__APIPATH__##/filter/##__ID__##.json" method="post"><div class="search-image"></div><div class="fieldgroup"><div class="field"><label for="search">Search:</label><input name="searchText" type="text" id="searchText" /><div class="searchButton"><input value="Go" name="action" type="submit" id="searchButton" /><input value="Clear" name="action" type="submit" id="clearSearchButton" /></div></div></div></form></div></div><div class="listContent"><div class="region" id="mustacheReportRegion-##__ID__##">##__DATA__##</div><script type="text/javascript">function doReport() { var template="<table id=\"mustache_table\" class=\"rounded-corner\"><thead>{{#header}}<tr class=\"tablerowheader\">{{#cells}}<th class=\"tablecell\">{{value}}</th>{{/cells}}</tr>{{/header}}</thead><tbody>{{#body}}<tr class=\"tablerowdata\">{{#cells}}<td class=\"tablecell\">{{value}}</td>{{/cells}}</tr>{{/body}}</tbody></table></div>"; var data = $.ajax({url: "##__APIPATH__##/filter/##__ID__##.json", cache:false , success: function(response){ var html = Mustache.to_html(template, response); $("#mustacheReportRegion-##__ID__##").html(html); }} ); }; $( document ).ready(doReport()); </script>';
